...
if [ -z "${X_BASE:-}" ]; then
	echo "x-coreutils: no launcher-free boot amalgam under $X_ROOT" >&2
	echo "  looked at boot/x-base.x and build/boot/x-base.x" >&2
	echo "  in a checkout, run 'coreutils boot' in the x-lang tree first" >&2
	exit 1
fi
...
# root armed exactly as run.x arms it.  Nothing under coreutils/ includes anything
